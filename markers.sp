...
{	
	if (buttons & IN_ATTACK2 && CanUseMarker(client))
	{
...
public int MenuHandler_Marker(Menu menu, MenuAction action, int param1, int param2)
{
	...
	if(action == MenuAction_Select)
	{
		...
		if(gA_MarkerSettings[param1][bMarkerDrawing])
		{
...
{
	return (IsValidClientTiny(client) && IsPlayerAlive(client) && client == get_warden_id());
}
